...
if (process.env.NODE_ENV === "development" && !API_BASE) {
  console.warn(
    "[api.js] NEXT_PUBLIC_API_BASE_URL is not set. " +
    "All API calls will fail. Set it in your .env.local file."
  );
}
...
export async function submitPublicLeadIntake({
  ...
  const route = intakeKey
    ? `/leads/intake/by-key/${encodeURIComponent(intakeKey)}`
    : `/leads/intake/${orgId}`;
...
export async function submitPublicDemoCallIntake(data, orgId = 1) {
  return fetch(
    `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/demo-call/intake/${orgId}`,
    {
      method: "POST",
...
export async function submitPublicDemoCallIntakeByKey(data, intakeKey) {
  return fetch(
    `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/demo-call/intake/by-key/${encodeURIComponent(intakeKey)}`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(data),
...
export async function submitPublicIntentIntake({
  intakeKey,
  orgId,
  ctaName,
  landingPage,
  ...
  const route = intakeKey
    ? `/api/leads/intake/intent/by-key/${encodeURIComponent(intakeKey)}`
    : `/api/leads/intake/intent/${orgId}`;
  ...
  return apiFetch(route, {
    ...
    body: JSON.stringify({
      ...
      referrer_url: referrerUrl || attribution?.referrer_url || null,
      utm_source: attribution?.utm_source || null,
      utm_medium: attribution?.utm_medium || null,
      utm_campaign: attribution?.utm_campaign || null,
      utm_term: attribution?.utm_term || null,
...
export async function submitPublicSupportChat({
  intakeKey,
  orgId,
  message,
  contextKey,
  ...
  const route = intakeKey
    ? `/api/leads/intake/support-chat/by-key/${encodeURIComponent(intakeKey)}`
    : `/api/leads/intake/support-chat/${orgId}`;
...
export async function getLeadRecoveryMetrics({ token }) {
  return apiFetch("/api/leads/recovery/metrics", {
    method: "GET",
    headers: {
      Authorization: `Bearer ${token}`,
    },
...
export async function getWebIntentKpi({ token, days = 7, limit = 10 }) {
  const query = new URLSearchParams({ days: String(days), limit: String(limit) });
  return apiFetch(`/api/reports/web-intent-kpi?${query.toString()}`, {
    method: "GET",
    headers: {
      Authorization: `Bearer ${token}`,
    },
...
export async function fetchDemoCallTranscript(callSid) {
  return apiFetch(`/api/demo-call/transcript/${encodeURIComponent(callSid)}`, {
    method: "GET",
  });
}

export async function sendDemoCallSmsSummary(leadId) {
  return apiFetch(`/api/demo-call/send-summary/${encodeURIComponent(leadId)}`, {
    method: "POST",
  });
}
